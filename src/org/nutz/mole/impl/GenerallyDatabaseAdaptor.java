...
public class GenerallyDatabaseAdaptor implements DatabaseAdaptor {
	...
	public MoleContext fromDb(MoleContext context) {
		...
		try {
			...
			para[0] = "TABLE";
			ResultSet tableResultset = metaData.getTables(	config.project.getProperty("db_catalog"),
															null,
															null,
															para);
			...
			for (ZTable zTable : tables) {
				...
				while (columnResultset.next()) {
					...
					zField.fieldName = config.tableFieldMapping.getProperty(zTable.getTableName()
																			+ "."
																			+ zField.dbFieldName);
					if (zField.fieldName == null)
						zField.fieldName = config.tableFieldMapping.getProperty("*."
																				+ zField.dbFieldName);
					if (zField.fieldName == null)
					...
					if (zField.classTypeName == null) {
						logger.warnf(	"Unkown type_name %s.%s, skip!",
										zTable.getTableName(),
										zField.dbFieldName);
						continue;
					}
